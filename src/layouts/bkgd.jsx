...
const StarBackground = () => {
  ...
  const generateStarData = useCallback(() => {
    ...
    const starColor = getComputedStyle(document.documentElement).getPropertyValue('--customwhite') || '#ffffff';
  ...
  const setupCanvas = useCallback((canvas, stars, size) => {
    if (!canvas) return;
  ...
  const animateStars = useCallback((canvas, stars, size, speed, posRef) => {
  ...
  useEffect(() => {
    ...
    const smallStarsAnimId = animateStars(smallStarsRef.current, starData.small, 1, 0.3, smallStarsPos);
    const mediumStarsAnimId = animateStars(mediumStarsRef.current, starData.medium, 2, 0.2, mediumStarsPos);
    const largeStarsAnimId = animateStars(largeStarsRef.current, starData.large, 3, 0.1, largeStarsPos);
